...
$categorySummary=[]; try { $categorySummary=db()->query("SELECT c.id,c.name,c.character_name,c.status,COUNT(DISTINCT s.spot_id) spots,COALESCE(SUM(CASE WHEN s.is_active=1 THEN 1 ELSE 0 END),0) active_spots FROM video_categories c LEFT JOIN spot_campaigns s ON s.category_id=c.id GROUP BY c.id ORDER BY c.name")->fetchAll(); } catch (Throwable $e) { try { $categorySummary=db()->query("SELECT c.id,c.name,c.character_name,c.status,0 spots,0 active_spots FROM video_categories c ORDER BY c.name")->fetchAll(); } catch (Throwable $ignored) { $categorySummary=[]; } }
