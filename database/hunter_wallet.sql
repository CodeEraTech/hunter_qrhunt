...
CREATE TABLE spot_campaigns (spot_id VARCHAR(64) PRIMARY KEY, master_token_hash CHAR(64) NOT NULL UNIQUE, spot_name VARCHAR(255) NOT NULL, category_id BIGINT UNSIGNED NULL, question_mode ENUM('SINGLE','COLLECTION') NOT NULL DEFAULT 'SINGLE', question_id BIGINT UNSIGNED NULL, collection_id BIGINT UNSIGNED NULL, target_scope ENUM('ALL_PUBLIC','WHITELIST','NEW_USER') NOT NULL DEFAULT 'ALL_PUBLIC', whitelisted_users JSON NULL, city_zone VARCHAR(128) NULL, venue_name VARCHAR(255) NULL, gps_mode ENUM('GPS_REQUIRED','DESK_STUDY') NOT NULL DEFAULT 'GPS_REQUIRED', latitude DECIMAL(10,7) NOT NULL DEFAULT 0, longitude DECIMAL(10,7) NOT NULL DEFAULT 0, proximity_alert_radius INT UNSIGNED NOT NULL DEFAULT 30, scanner_unlock_radius INT UNSIGNED NOT NULL DEFAULT 20, indoor_micro_clue TEXT NULL, reward_type ENUM('PROMO_CASH','COUPON_VOUCHER','DIRECT_CASH','MILESTONE') NOT NULL DEFAULT 'PROMO_CASH', reward_amount DECIMAL(18,2) NOT NULL DEFAULT 15, min_cart_value DECIMAL(18,2) NOT NULL DEFAULT 300, ttl_minutes INT UNSIGNED NOT NULL DEFAULT 30, auto_reversal TINYINT(1) NOT NULL DEFAULT 1, max_budget_scans INT UNSIGNED NOT NULL DEFAULT 500, scan_frequency_policy ENUM('PER_DEVICE','SINGLE_USE','DAILY','POOL') NOT NULL DEFAULT 'PER_DEVICE', is_active TINYINT(1) NOT NULL DEFAULT 1, created_by INT UNSIGNED NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY(category_id) REFERENCES video_categories(id), FOREIGN KEY(created_by) REFERENCES admins(id), CHECK(latitude BETWEEN -90 AND 90), CHECK(longitude BETWEEN -180 AND 180), CHECK(scanner_unlock_radius<=proximity_alert_radius), INDEX idx_spot_active(is_active,category_id)) ENGINE=InnoDB;
CREATE TABLE riddle_pool (riddle_id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, spot_id VARCHAR(64) NOT NULL, question_mode ENUM('SINGLE','MULTIPLE') NOT NULL DEFAULT 'SINGLE', riddle_text TEXT NOT NULL, instruction_text TEXT NULL, primary_answer VARCHAR(128) NOT NULL, synonyms JSON NOT NULL, yt_video_id VARCHAR(32) NULL, channel_url VARCHAR(500) NULL, answer_time_seconds INT UNSIGNED NOT NULL DEFAULT 60, weight_percentage INT UNSIGNED NOT NULL DEFAULT 100, status ENUM('ACTIVE','INACTIVE') NOT NULL DEFAULT 'ACTIVE', created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY(spot_id) REFERENCES spot_campaigns(spot_id) ON DELETE CASCADE, INDEX idx_riddle_spot(spot_id,weight_percentage,status)) ENGINE=InnoDB;
CREATE TABLE question_collections (collection_id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, collection_name VARCHAR(190) NOT NULL, spot_id VARCHAR(64) NOT NULL, description TEXT NULL, rotation_method ENUM('SEQUENTIAL','RANDOM','RANDOM_NO_REPEAT','ROUND_ROBIN','WEIGHTED') NOT NULL DEFAULT 'SEQUENTIAL', questions_per_session INT UNSIGNED NOT NULL DEFAULT 1, status ENUM('ACTIVE','INACTIVE') NOT NULL DEFAULT 'ACTIVE', created_by INT UNSIGNED NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY(spot_id) REFERENCES spot_campaigns(spot_id) ON DELETE CASCADE, FOREIGN KEY(created_by) REFERENCES admins(id), INDEX idx_collection_status(status,spot_id)) ENGINE=InnoDB;
CREATE TABLE question_collection_items (collection_id BIGINT UNSIGNED NOT NULL, riddle_id BIGINT UNSIGNED NOT NULL, question_order INT UNSIGNED NOT NULL DEFAULT 1, PRIMARY KEY(collection_id,riddle_id), UNIQUE KEY uq_collection_order(collection_id,question_order), FOREIGN KEY(collection_id) REFERENCES question_collections(collection_id) ON DELETE CASCADE, FOREIGN KEY(riddle_id) REFERENCES riddle_pool(riddle_id) ON DELETE CASCADE) ENGINE=InnoDB;
ALTER TABLE spot_campaigns ADD CONSTRAINT fk_spot_question FOREIGN KEY(question_id) REFERENCES riddle_pool(riddle_id) ON DELETE SET NULL, ADD CONSTRAINT fk_spot_collection FOREIGN KEY(collection_id) REFERENCES question_collections(collection_id) ON DELETE SET NULL;
...
CREATE TABLE qr_batches (
 ...
 category ENUM('VIDEO_PAHELI','PRODUCT_SKU','COUPON_VOUCHER','BONUS_POINT') NOT NULL, question_bank_category VARCHAR(190) NULL, generation_mode ENUM('UPLOAD','AUTO_COUNT') NOT NULL,
...
SELECT id,'Admin','admin','admin@gmail.com','$2y$12$87rXm9h5fJ/.G7toaNkvmuJpMcl/VBru5iMQPogg1MWrUvOdSm9Vm','ACTIVE',0
